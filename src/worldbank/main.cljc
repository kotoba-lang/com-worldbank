(ns worldbank.main
  "Kotodama WASM entrypoint for the Worldbank clean-room actor (L4) — Clojure port.
  ...
  facts (`worldbank.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "worldbank")
(def tier "L4")
...
(def entity-specs
  [{:entity "Country"     :plural "countries"     :id-prefix "worldban_cou"
    :fields [:iso2Code :name :capitalCity :incomeLevelId :lendingTypeId :regionId :longitude :latitude]
    :required [:iso2Code]
    :coerce {}                       :refs {:incomeLevelId "IncomeLevel" :lendingTypeId "LendingType"}}
   {:entity "Indicator"   :plural "indicators"    :id-prefix "worldban_ind"
    :fields [:name :unit :sourceId :sourceNote :sourceOrganization]
    :required [:name]
    :coerce {}                       :refs {:sourceId "Source"}}
   {:entity "Source"      :plural "sources"       :id-prefix "worldban_sou"
    :fields [:name :code :lastUpdated :dataAvailability :concepts]
    :required [:name]
    :coerce {:dataAvailability :bool :concepts :int} :refs {}}
   {:entity "IncomeLevel" :plural "incomelevels" :id-prefix "worldban_inc"
    :fields [:name]                         :required [:name]
    :coerce {}                       :refs {}}
   {:entity "LendingType" :plural "lendingtypes" :id-prefix "worldban_len"
    :fields [:name]                         :required [:name]
    :coerce {}                       :refs {}}
   {:entity "Topic"       :plural "topics"       :id-prefix "worldban_top"
    :fields [:name]                         :required [:name]
    :coerce {}                       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"worldbank.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "worldbank-compat" :tier tier :entities entities} 200])
...
(defn start! [] :worldbank-compat/ready)
